...
export const fileToBase64 = async (uri) => {
  // react-native-fs reads from a filesystem path; strip the file:// scheme and
  // decode %20-style escapes so paths with spaces resolve correctly.
  const path = uri.startsWith('file://') ? uri.replace('file://', '') : uri;
  return RNFS.readFile(decodeURIComponent(path), 'base64');
};
