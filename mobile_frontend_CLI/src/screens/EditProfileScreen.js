...
export default function EditProfileScreen({ navigation }) {
  ...
  const pickImage = async () => {
    // launchImageLibrary uses the system picker, which needs no runtime permission
    // on modern Android; on iOS it triggers the photo-access prompt automatically.
    const res = await launchImageLibrary({ mediaType: 'photo', quality: 0.5, includeBase64: true });
    if (res.didCancel) return;
    if (res.errorCode) return Alert.alert('Could not open gallery', res.errorMessage || res.errorCode);
    const asset = res.assets?.[0];
    if (asset?.base64) setPicture(asset.base64);
  };
